...
const getItems = async (req, res) => {
  try {
    ...
    const data = await fs.promises.readFile('./db/games.json', 'utf8')
...
const patchItem = async (req, res) => {
  try {
    ...
    await fs.promises.writeFile('./db/games.json', JSON.stringify(listToSave))
...
const postItem = async (req, res) => {
  try {
    ...
    if (!itemToSave.titre || itemToSave.titre.trim().length < 3) {
      res.status(400).json({ error: true, message: 'Missing titre or titre too short' })
      return
    }
    if (!itemToSave.genre || itemToSave.genre.trim().length < 3) {
      res.status(400).json({ error: true, message: 'Missing genre or genre too short' })
      return
    }
    if (!itemToSave.note || itemToSave.note <= 0) {
      res.status(400).json({ error: true, message: 'Missing note' })
      return
    }
    ...
    // sauvegarder la liste des données
    await fs.promises.writeFile('./db/games.json', JSON.stringify(list))
...
const deleteItem = async (req, res) => {
  try {
    ...
    res.json({ error: false, message: 'Game deleted' })
  } catch (err) {
    // en cas d'erreur, on renvoie un message d'erreur
    console.log(err)
    res.status(500).json({ error: true, message:'Error reading file' })
